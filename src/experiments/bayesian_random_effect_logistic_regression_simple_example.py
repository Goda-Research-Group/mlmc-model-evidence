...
def main():
    ...
    for t in range(4001):
        ...
        batch = np.random.choice(np.arange(N_total), B * 2)
        x = X[batch]
        y = Y[batch]
        with tf.GradientTape() as g:
            g.watch(params)
            # loss = - model.IWELBO_MLMC(x, y, max_level=9, w0=0.9, b=1.5, randomize=False)
            # loss = - model.IWELBO(x, y, n_MC=512) # this gives nested MC estimator
    ...
    # Print out results
    print("Results by LMELBO:")
    print("                \tbeta0\tbeta1\tbeta2\tbeta3")
    print("ground truth    \t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}".format(beta0, *list(beta)))
