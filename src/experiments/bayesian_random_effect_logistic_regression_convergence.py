...
def conv_stats_grad_dIWELBO(level, model, beta0, beta, alpha, params):
    ...
    def get_grad(x, y):
        ...
        with tf.GradientTape(persistent=True) as g:
            ...
            sigma = q_param['sigma']
            z = norm(loc=mu, scale=sigma).rvs([n_MC, N])
            diwelbos = model.pointwise_dIWELBO(x, y, z, q_param)
            iwelbos = model.pointwise_IWELBO(x, y, z, q_param)    
    ...
    args = [tf.expand_dims(arg, axis=1) for arg in [x, y]]
    # grads = tf.vectorized_map(get_grad, args)
    grads = [get_grad(x, y) for x, y in zip(*args)]
    grads0 = [v[0] for v in grads]
    grads0 = [np.array([v[i] for v in grads0]) for i in range(5)]
    grads1 = [v[1] for v in grads]
    grads1 = [np.array([v[i] for v in grads1]) for i in range(5)]
    grad_diwelbos = np.concatenate([
        np.expand_dims(g,1) if len(g.shape)<2 else g 
        for g in grads0
    ], axis=1)#[:D+1]
    grad_iwelbos = np.concatenate([
        np.expand_dims(g,1) if len(g.shape)<2 else g 
        for g in grads1
    ], axis=1)#[:D+1]
...
def main():
    ...
    for t in range(400):
        ...
        with tf.GradientTape() as g:
            ...
            # loss = - model.IWELBO(x, y, n_MC=1) # this gives nested MC estimator
    ...
    plt.yscale('log')
    # save fig
    filename = '../../out/bayesian_random_effect_logistic_regression/dLMELBO_convergence_{}.pdf'.format(timestamp())
    ...
    # save fig
    filename = '../../out/bayesian_random_effect_logistic_regression/dLMELBO_gradients_convergence_{}.pdf'.format(timestamp())
    plt.savefig(filename)
    print("saved the results to:\n {}\n".format(filename))
